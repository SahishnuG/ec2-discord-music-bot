...
def get_system_content(prompt_name: str, prompt_type: str = "chat", label: str | None = None) -> str:
    """Fetch the system message content robustly for chat or text prompts."""
    p = langfuse.get_prompt(name=prompt_name, type=prompt_type, label=label)
    raw = p.prompt
...
# Fetch fresh values
router_sys     = get_system_content("router",     prompt_type="chat")      # change to "text" if stored as text in langfuse
summariser_sys = get_system_content("summariser", prompt_type="chat")      # change to "text" if stored as text in langfuse
...
set_key(ENV_PATH, "ROUTER_SYSTEM_PROMPT",    json.dumps(router_sys))
set_key(ENV_PATH, "SUMMARISER_SYSTEM_PROMPT", json.dumps(summariser_sys))
