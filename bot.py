...
# Intents: enable message content so the bot can read messages for prefix commands
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game(name="say !ping"))
...
# A simple slash command (works alongside prefix commands)
...
async def setup_hook():
    # Optionally speed up slash-command propagation for a single server during development
    ...
    if guild_id:
        ...
        # Global sync (can take ~1 hour the first time)
        await bot.tree.sync()


# Silence useless bug report messages; yt_dlp sometimes calls this with kwargs
...
def main():
    ...
    bot.run(TOKEN, log_handler=None)  # use logging module config
